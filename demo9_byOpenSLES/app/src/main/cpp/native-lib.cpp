...
//创建播放器
void createPlayer(JNIEnv *env,jstring input){
    //初始化ffmpeg
    unsigned int rate;
    unsigned int channels;
    createFFmpeg(env,(int *)&rate,(int *)&channels,input);
    LOGE("RATE %d",rate);
...
Java_com_example_ffmpeg_1byopensles_MusicPlay_play(JNIEnv *env, jobject thiz, jstring input) {
    createEngine();             //创建引擎
    createMixVolume();          //创建混音器
    createPlayer(env,input);    //创建播放器
}


extern "C"
JNIEXPORT void JNICALL
Java_com_example_ffmpeg_1byopensles_MusicPlay_stop(JNIEnv *env, jobject thiz) {
    realseResource();
}